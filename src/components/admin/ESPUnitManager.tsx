...
export function ESPUnitManager({ requestId, studentName, studentEmail }: ESPUnitManagerProps) {
    ...
    const notifyUser = (unitName: string, isCompleted: boolean, note: string) => {
        // unitName is like "Unit 1", "Unit 2". Find proper title.
        const unitId = unitName.split(' ')[1];
        const unitDef = ESP_UNITS.find(u => u.id.toString() === unitId);
        const displayTitle = unitDef ? `UNIT ${unitId}: ${unitDef.title}` : unitName;

        fetch('/api/admin/notify-esp-unit', {
            method: 'POST',
    ...
    const handleToggle = async (unitName: string) => {
        ...
        // Notify user about toggle
        notifyUser(unitName, newState, current.admin_note);
    };
    ...
    const handleSaveNote = async (unitName: string) => {
        ...
        // Notify user about note change
        notifyUser(unitName, current.is_completed, current.admin_note);
    };
