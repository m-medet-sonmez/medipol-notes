...
export function MaterialUploadForm() {
    ...
    async function onSubmit(data: UploadFormData) {
        ...
        try {
            ...
            // E-posta bildirimi gönder (arka planda çalışsın, bekletmesin)
            fetch('/api/admin/notify-material', {
                method: 'POST',
                ...
                if (data.successCount > 0) {
                    ...
                    toast.error(`E-postalar gönderilemedi: ${data.message}`);
                } else if (!data.success) {
                    toast.error(`Mail hatası: ${data.error || 'Bilinmeyen hata'}`);
                } else {
                    toast.info(data.message || 'Öğrenci bulunamadı, mail atılmadı.');
                }
            }).catch(err => console.error('Bildirim gönderilirken hata:', err));
            ...
            window.dispatchEvent(new Event('material-uploaded'));
