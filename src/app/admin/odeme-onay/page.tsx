...
export default function PaymentApprovalsPage() {
    ...
    const handleApprove = async (request: PaymentRequest) => {
        ...
        try {
            ...
            // Başarılı olursa arka planda maile haber ver
            fetch('/api/admin/notify-approval', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ targetUserId: request.user_id, status: 'approved' })
            }).then(res => res.json())
                .then(data => {
                    if (data.success) {
                        toast.success('Öğrenciye onaylandı e-postası gönderildi!');
                    }
                })
                .catch(err => console.error('Onay e-postası hatası:', err));
    ...
    const handleReject = async (request: PaymentRequest) => {
        ...
        try {
            ...
            // Başarısız olursa arka planda maile haber ver
            fetch('/api/admin/notify-approval', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ targetUserId: request.user_id, status: 'rejected' })
            }).then(res => res.json())
                .then(data => {
                    if (data.success) {
                        toast.success('Öğrenciye reddedildiğine dair e-posta gönderildi!');
                    }
                })
                .catch(err => console.error('Ret e-postası hatası:', err));
